...
import { LanguageService as HtmlLanguageService, TokenType, getLanguageService } from 'vscode-html-languageservice';
...
export function activate(context: vscode.ExtensionContext) {
	function ensureAttribute(htmlLanguageService: HtmlLanguageService, document: vscode.TextDocument, position: vscode.Position) {
			...
			switch (token) {
				...
				case TokenType.AttributeValue:
					...
					if (lastAttributeName === 'class') {
						...
						if (
							offset > scanner.getTokenOffset() && offset < scanner.getTokenEnd()
						) {
							return true;
						}
					}
	...
	async function parseCss(cssLanguageService: cssLanguageService, htmlDocument: vscode.TextDocument) {
		/* 构造 css 文件的路径 */
	...
	const provider = {
		async provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: any, context: any): Promise<any> {
			...
			return Array.from(raw).map(
				(selector) => new vscode.CompletionItem(selector, vscode.CompletionItemKind.Color)
			);
		},
	}
